...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    if (product1Result.length === 0 || product2Result.length === 0 || product3Result.length === 0) {
      console.warn('One or more required products not found. Skipping order seeding.');
      return;
    }
    ...
    await queryInterface.bulkInsert('Orders', [{
      user_id: userId,
      total_amount: (parseFloat(product2Price) * 2 + parseFloat(product3Price) * 1).toFixed(2),
      status: 'processing',
      shipping_address_id: addressId,
      payment_status: 'paid',
    ...
    await queryInterface.bulkInsert('Payments', [{
      ...
      amount: (parseFloat(product2Price) * 2 + parseFloat(product3Price) * 1).toFixed(2),
      method: 'CreditCard',
      status: 'success',
