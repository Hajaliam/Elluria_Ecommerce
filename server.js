...
app.use(cors({ // CORS برای درخواست‌های HTTP
...
app.use(expressWinston.logger({
    ...
    ignoreRoute: function (req, res) { return false; } // تابع برای نادیده گرفتن روت‌ها (فعلا همه را لاگ می‌کند)
}));
...
io.on('connection', (socket) => {
    ...
    socket.on('sendAdviceMessage', async (data) => {
        ...
        logger.info(`Received advice message from ${userId ? 'User ID: ' + userId : 'Session ID: ' + sessionId}: ${sanitizedMessage}`);
        ...
        try {
            ...
            if (finalUserId) {
                ...
                logger.warn('Neither User ID nor Session ID found in message payload. Rejecting message.');
                socket.emit('receiveAdviceMessage', { sender: 'AI', message: 'Please refresh the page to start a new chat session.' });
                return;
            }

            const chatKey = finalUserId ? `user:${finalUserId}` : `session:${finalSessionId}`;
            let currentConversationHistory = conversationHistoryMap.get(chatKey) || [];
            ...
            currentConversationHistory.push({ sender: 'user', message: sanitizedMessage });
            conversationHistoryMap.set(chatKey, currentConversationHistory);

            const aiResponseText = await getAIResponse(sanitizedMessage, currentConversationHistory);

            currentConversationHistory.push({ sender: 'AI', message: aiResponseText });
            conversationHistoryMap.set(chatKey, currentConversationHistory);
            ...
            socket.emit('receiveAdviceMessage', { sender: 'مشاور فروش لامورا', message: aiResponseText });
        } catch (error) {
            logger.error(`Error processing or saving advice message/response: ${error.message}`, { stack: error.stack, userId: finalUserId, sessionId: finalSessionId });
            socket.emit('receiveAdviceMessage', { sender: 'مشاور فروش لامورا ', message: 'Sorry, I am unable to process your request at the moment. Please try again later.' });
        }
    });
...
app.use((err, req, res, next) => {
    logger.error(`Unhandled error: ${err.message}`, { stack: err.stack, path: req.path, ip: req.ip });
...
server.listen(PORT, () => { // سرور HTTP (که Socket.IO روی آن فعال است) را listen می‌کند
