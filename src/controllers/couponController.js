...
exports.createCoupon = async (req, res) => {
    let { code, discount_type, discount_value, min_amount, usage_limit, expiry_date, isActive } = req.body; // 👈 از let استفاده کنید
    ...
    try {
        ...
        const newCoupon = await Coupon.create({
            ...
            usage_limit, expiry_date, isActive
        });
...
exports.getAllCoupons = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Server error fetching coupons', error: error.message });
    }
};
...
exports.updateCoupon = async (req, res) => {
    ...
    let { new_code, discount_type, discount_value, min_amount, usage_limit, expiry_date, isActive } = req.body; // 👈 از let استفاده کنید
    ...
    try {
        ...
        if (new_code && new_code !== coupon.code) {
            const existingCoupon = await Coupon.findOne({ where: { code: new_code } });
            if (existingCoupon) {
        ...
        coupon.min_amount = min_amount !== undefined ? min_amount : coupon.min_amount;
...
exports.deleteCoupon = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Server error deleting coupon', error: error.message });
    }
};