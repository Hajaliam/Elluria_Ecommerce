...
const getOrCreateCart = async (req, res) => { // 👈 اینجا باید req و res را دریافت کند
    ...
    if (userId) {
        ...
        if (sessionId && !cart) {
            const guestCart = await Cart.findOne({ where: { session_id: sessionId } });
            if (guestCart) {
...
exports.addItemToCart = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Item added to cart successfully!', cartItem: cartItem, sessionId: sessionId });
    } catch (error) {
...
exports.updateCartItemQuantity = async (req, res) => {
    ...
    if (quantity <= 0) {
        return res.status(400).json({ message: 'Quantity must be positive. Use delete to remove item.' });
    }
    ...
    try {
        ...
        if (!cartItem) {
            return res.status(404).json({ message: 'Cart item not found or does not belong to your current cart.' });
        }

        if (cartItem.product.stock_quantity < quantity) {
            return res.status(400).json({ message: `Not enough product in stock. Available: ${cartItem.product.stock_quantity}` });
        }
        ...
        res.status(200).json({ message: 'Cart item quantity updated successfully!', cartItem: cartItem });
    } catch (error) {
